...
lPS();}
const sc=[];sc.push({"script_path":"bonk/default.1757524946.js","pct":100})
if(fgp("lngtd_version")){let scriptPath=`bonk/${fgp("lngtd_version")}.js`;lw(scriptPath);}else{const exp=sc.flatMap(co=>Array(co.pct).fill(co));const w=exp[Math.floor(Math.random()*exp.length)];lw(w.script_path);}}());